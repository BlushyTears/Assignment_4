...
int UnitBase::getcurrentCorrespondingTile(std::vector<Vector2>& pathToCheck, Vector2& _unitPos) {
	...
	int closestIdx = 0;
	for (int i = 0; i < (int)pathToCheck.size(); i++) {
...
void Builder::commandUnit() {
	if (targetBuilding == nullptr) {
		for (auto& building : this->buildings) {
			if (!building->isBuilt && !building->isBuilding) {
	...
	if (targetBuilding != nullptr) {
		if (currentPath.size() == 0) {
			...
			if (connectionIdx >= (int)currentPath.size()) {
				if (targetBuilding->treeCount < targetBuilding->minTreesNeeded) {
					return;
				}

				if (ArmSmith* armSmith = dynamic_cast<ArmSmith*>(targetBuilding)) {
					if (armSmith->ironArrowCount < armSmith->ironArrowsNeeded) {
						return;
					}
				}
